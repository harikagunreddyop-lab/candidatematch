...
export default function CompanyJobDetailPage() {
  ...
  if (!job) return (
    <div className="max-w-2xl mx-auto px-4 py-16 text-center text-surface-500">
      ...
      <Link href="/dashboard/company/jobs" className="block mt-2 text-violet-400 hover:underline">Back to jobs</Link>
    </div>
  );
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="font-semibold text-white">Applicants</h2>
        <span className="text-surface-500 text-sm">{applicationsCount} applications</span>
      </div>
      <Link href={`/dashboard/company/jobs/${jobId}/candidates`} className="flex items-center gap-3 p-4 rounded-xl border border-surface-700 bg-surface-800/50 hover:bg-surface-700/50 transition-colors">
