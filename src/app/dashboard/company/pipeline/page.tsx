...
export default function CompanyPipelinePage() {
  ...
  const load = useCallback(async () => {
    ...
    if (!session) { setLoadError('Not signed in'); setLoading(false); return; }
    ...
    const { data: companyJobs } = await supabase.from('jobs').select('id').eq('company_id', profile.company_id);
    ...
    const [appRes, matchRes] = await Promise.all([
      supabase.from('applications').select('*, candidate:candidates(id, full_name, primary_title, location, rating), job:jobs(id, title, company, location, url)').in('job_id', jobIds).order('updated_at', { ascending: false }),
      supabase.from('candidate_job_matches').select('*, candidate:candidates(id, full_name, primary_title, location, rating), job:jobs(id, title, company, location, url)').in('job_id', jobIds).gte('fit_score', 50).order('fit_score', { ascending: false }).limit(50),
    ]);
    ...
    const appliedKeys = new Set((appRes.data || []).map((a: any) => `${a.candidate_id}:${a.job_id}`));
    for (const m of matchRes.data || []) {
      if (!appliedKeys.has(`${m.candidate_id}:${m.job_id}`)) grouped['ready'].push({ ...m, _type: 'match' });
    }
    setCards(grouped);
    setLoading(false);
  ...
  useEffect(() => { load(); }, [load]);
  useEffect(() => {
    const ch = supabase.channel('company-pipeline')
  ...
  const moveCard = async (card: any, toStage: string) => {
    setMovingCardId(card.id || `${card.candidate_id}:${card.job_id}`);
    try {
      if (card._type === 'match') {
        const res = await fetch('/api/applications', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ candidate_id: card.candidate_id, job_id: card.job_id, status: toStage, applied_at: toStage === 'applied' ? new Date().toISOString() : null }) });
        const data = await res.json().catch(() => ({}));
        if (!res.ok) { toast(data.error || 'Failed to create application', 'error'); return; }
      } else {
        const res = await fetch('/api/applications', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: card.id, status: toStage }) });
        const data = await res.json().catch(() => ({}));
        if (!res.ok) { toast(data.error || 'Failed to update status', 'error'); return; }
      }
      await load();
    } catch (e: any) { toast(e.message || 'Failed to move card', 'error'); }
    finally { setMovingCardId(null); }
  };

  const onDrop = async (toStage: string) => {
    if (!dragging || dragging.fromStage === toStage || movingCardId) { setDragging(null); setDragOver(null); return; }
  ...
  if (loadError) return (
    <div className="card p-12 text-center max-w-md mx-auto">
      <p className="text-surface-700 dark:text-surface-300 font-medium">Failed to load pipeline</p>
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <h1 className="text-2xl font-bold text-surface-900 dark:text-surface-100 font-display">Pipeline</h1>
        <p className="text-sm text-surface-500 dark:text-surface-400 mt-1">{totalCards} applications to your company&apos;s jobs · drag to update status</p>
      </div>
      <div className="flex gap-3 overflow-x-auto pb-4 min-h-[70vh]">
        {STAGES.map(stage => {
          ...
          return (
            <div key={stage.key} className={cn('flex-shrink-0 w-64 rounded-xl flex flex-col transition-colors', stage.color, isOver && 'ring-2 ring-brand-400 dark:ring-brand-500')}
              ...
              <div className="flex-1 p-2 space-y-2 overflow-y-auto">
                {stageCards.map((card: any) => (
                  <CompanyPipelineCard key={card.id || `${card.candidate_id}:${card.job_id}`} card={card} stage={stage.key} disabled={!!movingCardId}
                    onDragStart={() => { if (!movingCardId) setDragging({ card, fromStage: stage.key }); }} onMove={moveCard} />
                ))}
                {stageCards.length === 0 && <div className={cn('h-16 rounded-lg border-2 border-dashed flex items-center justify-center text-xs', isOver ? 'border-brand-400 dark:border-brand-500 text-brand-500 dark:text-brand-400' : 'border-surface-200 dark:border-surface-600 text-surface-300 dark:text-surface-500')}>{isOver ? 'Drop here' : 'Empty'}</div>}
              </div>
            </div>
          );
        })}
...
function CompanyPipelineCard({ card, stage, disabled, onDragStart, onMove }: { card: any; stage: string; disabled?: boolean; onDragStart: () => void; onMove: (card: any, stage: string) => void }) {
  const candidate = card.candidate;
  const job = card.job;
  const isMatch = card._type === 'match';
  const NEXT_STAGE: Record<string, string | null> = { ready: 'applied', applied: 'screening', screening: 'interview', interview: 'offer', offer: null, rejected: null };
  const NEXT_LABELS: Record<string, string> = { ready: '→ Apply', applied: '→ Screen', screening: '→ Interview', interview: '→ Offer' };
    ...
    <div draggable={!disabled} onDragStart={disabled ? undefined : onDragStart}
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          <Link href={`/dashboard/company/candidates/${card.candidate_id}`} className="text-xs font-semibold text-surface-900 dark:text-surface-100 hover:text-brand-600 dark:hover:text-brand-400 leading-tight truncate block" onClick={e => e.stopPropagation()}>{candidate?.full_name}</Link>
