...
export default function RecruiterCandidateDetail() {
  ...
  const runAtsForJob = async (jobId: string) => {
    ...
    try {
      ...
      // Update only the relevant match in-place so the whole page does not reload
  ...
  return (
    <div className="space-y-6 min-w-0 max-w-full overflow-x-hidden">
      ...
      <nav className="flex items-center gap-1.5 text-sm text-surface-500 mb-2" aria-label="Breadcrumb">
        <Link href="/dashboard/recruiter/candidates" className="hover:text-surface-900 dark:hover:text-surface-100">Candidates</Link>
        <span>/</span>
          ...
          <div className="bg-white dark:bg-surface-800 rounded-2xl shadow-xl border border-surface-200 dark:border-surface-600 w-full max-w-2xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-surface-100 dark:border-surface-700">
              <div>
                ...
                <p className="text-xs text-surface-500 dark:text-surface-400 mt-0.5">Paste any job description and see this candidate&apos;s ATS score</p>
              </div>
