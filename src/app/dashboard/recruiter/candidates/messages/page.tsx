...
export default function CandidateMessagesPage() {
  ...
  const loadAvailableUsers = async () => {
    if (!profile) return;
    const { data: myCand } = await supabase.from('candidates').select('id').eq('user_id', profile.id).single();

    const { data: adminRows } = await supabase.from('profiles').select('*').eq('role', 'admin');
    const adminList = (adminRows || []).filter((u: any) => u.id !== profile.id);
    ...
    const { data: assignments } = await supabase
      .from('recruiter_candidate_assignments').select('recruiter_id').eq('candidate_id', myCand.id);
    const recruiterIds = (assignments || []).map((a: any) => a.recruiter_id).filter(Boolean) as string[];

    let recruiterProfiles: Profile[] = [];
    if (recruiterIds.length > 0) {
      const { data: profileRows } = await supabase.from('profiles').select('*').in('id', recruiterIds);
      recruiterProfiles = profileRows || [];

      const foundIds = new Set(recruiterProfiles.map((p: any) => p.id));
