...
import { BarChart2, Target, Star, AlertCircle, User, FileText, Zap, Sparkles, Info, X, Brain } from 'lucide-react';
...
export default function SkillReportTabContent() {
  ...
  useEffect(() => {
    if (!atsReportAllowed || !candidate || !matches?.length || autoAtsRef.ran) return;
    const missing = matches.filter((m: any) => typeof m.ats_score !== 'number').slice(0, 3);
    if (missing.length === 0) {
      autoAtsRef.ran = true;
      return;
    }
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- autoAtsRef/runAtsForJob intentional; run when atsReportAllowed, candidate, or matches change
  }, [atsReportAllowed, candidate, matches]);
  ...
  const profileStrengths = Object.entries(matchedFreq)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 12)
    .map(([k]) => k);
  const whatToAdd = Object.entries(missingFreq)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 10)
    .map(([k]) => k);
  ...
  const resumeAnalysis = resumes.map((r) => {
    ...
    const strongIn = Object.entries(kw)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 8)
      .map(([x]) => x);
    const bestFor = forResume
      .map((m: any) => ({ title: m.job?.title, company: m.job?.company, score: m.fit_score }))
  ...
  const profileOnlyMatches = matches.filter(
    (m: any) => !m.best_resume_id && !m.ats_resume_id && !(m.ats_checked_at && tailoredByJob[m.job_id])
  );
  ...
  if (matches.length === 0) {
    return (
      <EmptyState
        ...
        description="Run matching to see why you score high or low for each role. Your recruiter will add jobs and run the engine."
      />
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="space-y-4">
          {matches
            .filter((m) => m.fit_score >= SCORE_APPLY_OK)
            .slice(0, 5)
            .map((m) => {
